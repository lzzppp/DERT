...
def t2vec(args):
    ...
    if os.path.isfile(args.checkpoint):
        ...
        m0.eval()
        vecs = []
        scaner = DataOrderScaner(os.path.join(args.data, "trj-{}-{}{:.1f}.t".format(args.cityname, args.encode_data, args.grid_size)), args.t2vec_batch)
        scaner.load()
        i = 0
        encode_time = 0.0
        while True:
        ...
        path = os.path.join(args.data, "traj_emb/trj_{}_{}{:.0f}.h5".format(args.cityname, args.encode_data, args.grid_size))
