...
def run_single_dataset_experiments(args, device=None, USE_POI=False, model_type='S', unit=None):
    """跑自己生成的单个数据集上的实验"""
    
    if args.data_name == "porto" or args.data_name == "geolife" or args.data_name == "singapore" or args.data_name == "gowalla" or args.data_name == 'foursquare':
        with open(os.path.join(args.data_path, 'data_input1_180.pkl'), 'rb') as f:
            data_input = pickle.load(f)
    
    SN = SiameseNet(loc_size=args.vid_size, tim_size=24, loc_emb_size=args.loc_emb_size,
                           tim_emb_size=args.tim_emb_size, hidden_size=args.hidden_size,
                           batch_size=args.batch_size, device=device, loss_mode=args.loss_mode,
                           mod=args.rnn_mod, attn_mod=args.attn_mod, layers=args.layers, fusion=model_type,
                           poi_size=args.poi_size if USE_POI else None,
    ...
    SN_dense, rank_pre, hit_pre = run_siamese2(SN, args, data_input, args.lr_match,
                                                      train_mode='cross', device=device, USE_POI=USE_POI)

    print(rank_pre, hit_pre)

def run_siamese2(SN, args, data_input, LR, train_mode='cross',
                reproduction=False, SAVE_PATH=None, device=None, USE_POI=False, test_pretrain=False):
    """这个版本只为统计f1-score"""
    BATCH_SIZE = args.batch_size
    criterion, criterion2, cos_dis = None, None, None
    ...
    loss_records = {"training": [], "validation": [], "testing": []}
    metrics_records = {"training": {"acc": [], "rec": [], "f1": [], "auc": []},
                       "validation": {"rank": [], "hit": [], "list": [],"acc": [], "rec": [], "f1": [], "auc": []},
                       "testing": {"rank": [], "hit": [], "list": [],"acc": [], "rec": [], "f1": [], "auc": []}}
    if SAVE_PATH is None:
        SAVE_PATH = args.save_path + "/"
    tmp_path = 'checkpoint/'
    if not os.path.exists(SAVE_PATH + tmp_path):
    ...
    for e in tqdm(range(args.epoch), desc="EPOCH"):
        ...
        if reproduction is False:
            ...
            runtime = 0
            batch_end = time.time ()
            for i in range (0, testing_len, BATCH_SIZE):
                if i + BATCH_SIZE > testing_len:
                    continue
                samples = copy.deepcopy (data_input["test"][i:i + BATCH_SIZE])
                batch_data = gen_batch_similarity (samples, device=device, poi=USE_POI)
                batch_start = time.time ()
                datatime += batch_start - batch_end
                for _ in range (2):
            ...
            # metrics = {'TP': 0, 'TN': 0, 'FP': 0, 'FN': 0}
            # label_predict = {"label": [], "predict": []}
            # for i in range(0, validing_len, BATCH_SIZE):
            #     if i + BATCH_SIZE > validing_len:
            #         continue
            #     samples = copy.deepcopy(data_input["valid"][i:i + BATCH_SIZE])
            #     batch_data = gen_batch_similarity(samples, device=device, poi=USE_POI)
            #
            #     for _ in range(2):
            #         optimizer.zero_grad()
            ...
            #         label_predict["label"].extend(tg)
            #
            # metrics_records, loss_records, avg_loss, valid_f1 = collect_metrics(metrics_records, loss_records,
            #                                                           "validation", metrics, label_predict, total_loss, get_f1=True)
            # validation_f1s.append(valid_f1)
    
    # Testing
    mid = np.argmax(validation_f1s)
    SN.load_state_dict(torch.load(SAVE_PATH + tmp_path + str(mid) + '.m'))
    total_loss = []
    SN.train(False)
    metrics = {'TP': 0, 'TN': 0, 'FP': 0, 'FN': 0}
    ...
    metrics_records, loss_records, avg_loss = collect_metrics(metrics_records, loss_records,
                                                                      "testing", metrics, label_predict, total_loss)
