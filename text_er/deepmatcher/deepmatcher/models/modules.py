...
class Fusion(LazyModule):
    def _init(self, merge="concat-mul-diff", transfrom="2-layer-highway",
        ...
        self.merge_network = dm.modules._merge_module(merge)
        self.transform_network = dm.modules._transform_module(transfrom, hidden_size)
...
class PairAttention(LazyModule):
    def _init(self,
        ...
        hidden_size = hidden_size if hidden_size is not None else input_size[0]
        self.hidden_size = hidden_size
        self.alignment_network = dm.modules._alignment_module(alignment_network, hidden_size)
