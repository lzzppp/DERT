...
def savecheckpoint(state, is_best, cityname, grid_size):
    filename = "saved_model/checkpoint_%s_%s.pt" % (cityname, grid_size)
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'saved_model/best_model_%s_%s.pt' % (cityname, grid_size))
...
def train(args):
    ...
    trainsrc = os.path.join(args.data, "%s-%d-train.src" % (args.cityname, args.grid_size))
    traintrg = os.path.join(args.data, "%s-%d-train.trg" % (args.cityname, args.grid_size))
    ...
    valsrc = os.path.join(args.data, "%s-%d-val.src" % (args.cityname, args.grid_size))
    valtrg = os.path.join(args.data, "%s-%d-val.trg" % (args.cityname, args.grid_size))
    ...
    for iteration in range(args.start_iteration, num_iteration):
        try:
            ...
            if iteration % args.save_freq == 0 and iteration > 0:
                ...
                }, is_best, args.cityname, args.grid_size)
