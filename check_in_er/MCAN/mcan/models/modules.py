...
class NoMeta(nn.Module):
    """A wrapper module to allow regular modules to take
...
class Fusion(LazyModule):
    def _init(self, merge="concat-mul-diff", transfrom="2-layer-highway",
        ...
        self.merge_network = mcan.modules._merge_module(merge)
        self.transform_network = mcan.modules._transform_module(transfrom, hidden_size)
...
class SelfAttention(LazyModule):
    def _init(self,
        ...
        for head in range(heads):
            self.alignment_networks.append(
                mcan.modules._alignment_module(alignment_network, hidden_size))

        if value_transform_network is None and heads > 1:
            value_transform_network = mcan.modules.Transform(
                '1-layer-highway', non_linearity=None, hidden_size=hidden_size // heads)
        self.value_transform_network = mcan.modules._transform_module(
            value_transform_network, hidden_size // heads)

        self.value_merge = mcan.modules._merge_module(value_merge)

        self.softmax = nn.Softmax(dim=2)

        if output_transform_network is None and heads > 1:
            output_transform_network = mcan.modules.Transform(
                '1-layer-highway', non_linearity=None, hidden_size=hidden_size)
        self.output_transform_network = mcan.modules._transform_module(
        ...
        self.bypass_network = mcan.modules._bypass_module(bypass_network)
    ...
    def _forward(self, input_with_meta, raw_input_with_meta=None):
        input = self.input_dropout(input_with_meta.data)
...
class PairAttention(LazyModule):
    def _init(self,
        ...
        hidden_size = hidden_size if hidden_size is not None else input_size[0]
        self.hidden_size = hidden_size
        self.alignment_network = mcan.modules._alignment_module(alignment_network, hidden_size)
