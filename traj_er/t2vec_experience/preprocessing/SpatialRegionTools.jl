...
function createTrainVal(region::SpatialRegion,
    ...
    cellsize = Int(region.xstep)
    h5open(trjfile, "r") do f
        trainsrc, traintrg = open("../data/$cityname-$cellsize-train.src", "w"), open("../data/$cityname-$cellsize-train.trg", "w")
        validsrc, validtrg = open("../data/$cityname-$cellsize-val.src", "w"), open("../data/$cityname-$cellsize-val.trg", "w")
        for i = 1:ntrain+nval
            trip = f["/trips/$i"] |> read
