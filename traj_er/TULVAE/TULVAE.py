...
ap.add_argument("-dataset", type=str, default='geolife')
...
c_hidden=int(340 * bigger) #classifer embedding
bata=0.5
keep_prob = tf.placeholder("float")
alpha = tf.placeholder("float")
it_learning_rate= tf.placeholder("float")
z_size=int(50 * bigger)

### 修改的参数
#data set
label_size=2
# root_folder = '/mnt/data1/zhuzheng/trajectory/TULVAE_pair'
# exp_folder = os.path.join(root_folder, args.dataset, "%.1fh" % args.length)
exp_folder = os.path.join(os.getcwd().strip('TULVAE') + 'dataset/tulvae_dataset' + data)
poi_vector_file = os.path.join(exp_folder, 'embedding.dat')
traindata = os.path.join(exp_folder, 'train.dat')
testdata = os.path.join(exp_folder, 'test_add3.dat')

out_folder = os.path.join(exp_folder, 'output')
if not os.path.exists(out_folder):
    os.mkdir(out_folder)
temp_folder = os.path.join(exp_folder, 'temp')
...
def getXs(poi_vector_file):  # read POIs vector
    fpointvec = open(poi_vector_file, 'r')  #
    item = 0
...
def read_train_test_data(traindata, testdata):
    ftraindata = open(traindata, 'r')
    ftestdata = open(testdata, 'r')
    test_T = list()
    test_UserT = list()
    pointT = list()
    userT = list()
    ...
    for line in ftraindata.readlines():
        X = [str(element) for element in line.split()]
    ...
    for line in ftestdata.readlines():
        X = [str(element) for element in line.split()]
        test_T.append(X[1:])
        test_UserT.append(int(X[0]))
    ftraindata.close()
    ftestdata.close()
    print('train size:', len(pointT), 'test size:', len(test_UserT), 'total User number:', len(User_List))
...
def readtraindata():
    ...
    test_UserT = list()
    test_lens = list()
    ftraindata = open('data/brightkite_scopus_tra_1215.dat',
                      'r')
    tempT = list()  #
    pointT = list()  #
    userT = list()  #
...
#input
getXs(poi_vector_file)
pointT, userT,test_T, test_UserT,User_List=read_train_test_data(traindata, testdata)
...
def get_cost_u(u_encoder_embed_input,u_decoder_embed_input):
    ...
    #U=U_LOSS
    #print 'ucost',u_cost
    U = tf.reduce_sum(tf.multiply(L_ulab, prob_y),1) #- tf.multiply(prob_y, tf.log(prob_y)))  #
    #print 'YES',U
    return U#,L_ulab
...
#LABEL_LOSS=tf.reduce_mean(cost_l)
#
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainT) // batch_size:
                ...
                for i in range(len(pred_batch)):
                    ...
                    top5=np.argpartition(a=-value,kth=1)[:1]
                ...
                train_cost+=loss
                # #print 'result',c_pred
                # for result in c_pred:
                #     if result==True:
                #         acc+=1
                step+=1 # while
                count+=1
            all_runtime += run_time
            print(run_time)
            ...
            saver.save(sess, temp_folder+'/bk_tul_lvae.pkt')
        print("all run time is : ", all_runtime)
        save_metrics(Learning_rate, TEST_P, TEST_R, TEST_F1, TEST_ACC1, TEST_ACC5, root=out_folder+'/vae_ltests.txt')
        save_metrics(Learning_rate, TRAIN_P, TRAIN_R, TRAIN_F1, TRAIN_ACC1, TRAIN_ACC5,
                     root=out_folder+'vae_ltrains.txt')
...
def test_model(sess,testT,testU,epoch):
    ...
    while step < len(testT) // batch_size:  #
        ...
        step+=1 # while
    print("test time is : ", run_time)
    # Precision Recall, F1
    P = []
    R = []
    print('keys', TEST_DIC.keys())
    ...
    if percision != 0 and recall != 0:
        real_f1 = 2 * percision * recall / (percision + recall)
    else:
        real_f1 = 0
...
def save_metrics(LEARN_RATE,TRAIN_P,TRAIN_R,TRAIN_F1,TRAIN_ACC1,TRAIN_ACC5,root='result/bk_metric_vae.txt'):
    files=open(root,'a+')
...
def draw_pic_metric(TEST_P,Test_R,Test_F1,Test_ACC1,TEST_ACC5,name='train'):
    ...
    plt.show()
    plt.savefig(out_folder + '/out_%s.png'%name, dpi=300)
