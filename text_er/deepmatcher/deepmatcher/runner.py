...
class Runner(object):
    ...
    def _run(run_type,
        ...
        for batch_idx, batch in enumerate(run_iter):
            ...
            if run_type == "TEST":
                for (i, pred, label) in zip(batch._id, output.max(1)[1].data, getattr(batch, label_attr).data):
                    if pred != label:
                        wrong_list.append(i)
        ...
        if progress_style == 'tqdm-bar':
            ...
            sys.stderr.flush()
        if run_type == "TEST":
            with open("wrong_list.json", "w+", encoding="utf-8") as f:
                f.write(json.dumps(wrong_list))
    ...
    def eval(model, dataset, **kwargs):
        return Runner._run('TEST', model, dataset, **kwargs)
