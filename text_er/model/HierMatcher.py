...
from model.modules import LazyModule, RNN, SelfAttention, PairAttention, Fusion, GatingMechanism, GlobalAttention
...
class HierMatcher(dm.MatchingModel):
    ...
    def initialize(self, train_dataset, init_batch=None):
        ...
        self.TextSummarizer = TextSummarizer(hidden_size=2 * self.hidden_size)
        ...
        # self.left_fields = ['left_title', 'left_category', 'left_brand', 'left_modelno', 'left_price', 'left_value']
        # self.right_fields = ['right_title', 'right_category', 'right_brand', 'right_modelno', 'right_price', 'right_value'] WA
        
        self.left_fields = ['left_title', 'left_authors', 'left_venue', 'left_year', 'left_value']
        self.right_fields = ['right_title', 'right_authors', 'right_venue', 'right_year', 'right_value'] # DA DS
        
        # self.left_fields = ['left_title', 'left_manufacturer', 'left_price', 'left_value']
        # self.right_fields = ['right_title', 'right_manufacturer', 'right_price', 'right_value'] AG
        
        # self.left_fields = ['left_name', 'left_description', 'left_price', 'left_value']
        # self.right_fields = ['right_name', 'right_description', 'right_price', 'right_value'] AB
        ...
        # self.highway_layer = HighwayMLP(self.hidden_size * 2)

        len_linear_input = len_highway_input
        self.label = nn.Linear(len_linear_input, 2)
        # self.label = nn.Linear(2 * self.hidden_size, 2)

        self.init_field_representation()

        self.linear_token_compare = nn.Linear(2 * self.hidden_size,1)
        # self.global_attention = GlobalAttention(2 * self.hidden_size, use_meta=False)
    ...
    def representation_layer(self, input, fields, embeddings):
        ...
        for field in fields:
            ...
            value_embedding_list.append(AttrTensor.from_old_metadata(value_embedding, getattr(input, field)))

            # Get contextual representation by Bi-GRU
            output_rnn, _ = self.get_rnn_output(value_embedding)
            output_rnn_list.append(AttrTensor.from_old_metadata(output_rnn, getattr(input, field)))
    ...
    def forward(self, input):
        ...
        for i in range(0, len(self.left_fields)):
            ...
            output_rnn_right_all = torch.cat([t.data for t in output_rnn_right_list[:-1]], 1)
            # output_rnn_right_all = torch.cat([t.data for t in output_rnn_right_list], 1)

            output_rnn_left, output_rnn_right_all = self.TextSummarizer(output_rnn_left, output_rnn_right_all, right_token_mask_list[:-1])
            # output_rnn_left, output_rnn_right_all = self.TextSummarizer(output_rnn_left, output_rnn_right_all, right_token_mask_list)
            
            # Cross-attribute token alignment
            attention_matrix = self.token_level_attention(output_rnn_left, output_rnn_right_all)
            attention_matrix = self.process_pad_token(attention_matrix, right_token_mask_list[:-1])
        ...
        for i in range(0, len(self.right_fields)):
            ...
            output_rnn_left_all = torch.cat([t.data for t in output_rnn_left_list[:-1]], 1)
            # output_rnn_left_all = torch.cat([t.data for t in output_rnn_left_list], 1)
            
            output_rnn_right, output_rnn_left_all = self.TextSummarizer(output_rnn_right, output_rnn_left_all, left_token_mask_list[:-1])
            # output_rnn_right, output_rnn_left_all = self.TextSummarizer(output_rnn_right, output_rnn_left_all, left_token_mask_list)

            # Cross-attribute token alignment
            attention_matrix = self.token_level_attention(output_rnn_right, output_rnn_left_all )
            attention_matrix = self.process_pad_token(attention_matrix, left_token_mask_list[:-1])
...
class TextSummarizer(LazyModule):
    ...
    def _forward(self, left_input, right_input, right_mask):
        # left_contextualized = self.gru(left_input)
        # right_contextualized = self.gru(right_input)

        left_contextualized = self.self_attention(left_input)
        right_contextualized = self.self_attention(right_input, right_mask)

        left_fused = self.pair_attention(
            left_contextualized, right_contextualized, left_input, right_input, right_mask)
